...
export function matchTradeSymbol(text, symbols) {
  const token = String(text || "").trim().replace(/[.,;:]+$/g, "");
  const lower = token.toLowerCase();
  if (!lower || lower.length > 18 || STOP.has(lower)) return null;
  const hits = (symbols || []).filter((item) => item.abbr && item.abbr.toLowerCase() === lower && item.abbr.length >= 2 && item.category !== "Raceway");
...
export function buildAiMarks({
  ...
  for (const page of pages || []) {
    ...
    for (const token of page.tokens || []) {
      const symbol = matchTradeSymbol(token.text, symbols);
      if (!symbol) continue;
